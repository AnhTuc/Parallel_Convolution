		...
		uint8_t * outPixels)
{
	...
	if((r < height) && (c< width-2)){
		int idx_in = r * width + c;

		if(idx_in % 3==0){
		int idx_out= idx_in/3;
        uint8_t red = inPixels[idx_in];
        uint8_t green = inPixels[idx_in + 1];
        uint8_t blue = inPixels[idx_in + 2];
		outPixels[idx_out]=0.299*red + 0.587*green + 0.114*blue;
		} 
	}
		...
		bool useDevice=false, dim3 blockSize=dim3(1))
{
	...
	else // use device
	{
		...
		CHECK(cudaMalloc(&d_in1, nBytes*3));
        CHECK(cudaMalloc(&d_out, nBytes));

		// TODO: Copy data to device memories
		CHECK(cudaMemcpy(d_in1, inPixels, nBytes*3, cudaMemcpyHostToDevice));

		// TODO: Set grid size and call kernel (remember to check kernel error)
		dim3 gridSize((width*3 - 1) / blockSize.x + 1, 
                      (height - 1) / blockSize.y + 1);
        convertRgb2GrayKernel<<<gridSize, blockSize >>>(d_in1, width*3, height, d_out);
